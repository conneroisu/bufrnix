...
with lib; let
  ...
  roadrunnerModule =
    if cfg.roadrunner.enable
    then
      import ./roadrunner.nix {
        inherit pkgs lib;
        cfg =
          cfg.roadrunner
          // {
  ...
  # Framework modules
  laravelModule =
    if cfg.frameworks.laravel.enable
    then
      import ./frameworks/laravel.nix {
        inherit pkgs lib;
        cfg =
          cfg.frameworks.laravel
          // {
            inherit outputPath;
            namespace = cfg.namespace;
          };
  ...
  symfonyModule =
    if cfg.frameworks.symfony.enable
    then
      import ./frameworks/symfony.nix {
        inherit pkgs lib;
        cfg =
          cfg.frameworks.symfony
          // {
            inherit outputPath;
            namespace = cfg.namespace;
          };
  ...
  # Async module
  asyncModule =
    if (cfg.async.reactphp.enable || cfg.async.swoole.enable || cfg.async.fibers.enable)
    then
      import ./async.nix {
        inherit pkgs lib;
        cfg =
...
in {
  ...
  runtimeInputs =
    ...
    ++ (optional cfg.composer.enable pkgs.phpPackages.composer)
