...
abstract class HelloServiceAbstractClient
{
     ...
     * {@inheritdoc}
     */
    public function Hello(array $ctx, \Test\HelloRequest $in): \Test\HelloResponse
    {
        $ctx = Context::withPackageName($ctx, 'test');
        $ctx = Context::withServiceName($ctx, 'HelloService');
        $ctx = Context::withMethodName($ctx, 'Hello');

        $out = new \Test\HelloResponse();

        $url = $this->addr;
        if (empty($this->prefix)) {
            $url = $url.'/test.HelloService/Hello';
        } else {
            $url = $url.'/'.$this->prefix.'/test.HelloService/Hello';
        }
