...
/**
 * @internal HelloServiceAbstractClient provides abstraction for JsonClient and Client (default).
 ...
 */
abstract class HelloServiceAbstractClient
{
    /**
     * @var server
     */
     ...
     * {@inheritdoc}
     */
    public function Hello(array $ctx, \Example\V1\HelloRequest $in): \Example\V1\HelloResponse
    {
        $ctx = Context::withPackageName($ctx, 'example.v1');
        $ctx = Context::withServiceName($ctx, 'HelloService');
        $ctx = Context::withMethodName($ctx, 'Hello');

        $out = new \Example\V1\HelloResponse();

        $url = $this->addr;
        if (empty($this->prefix)) {
            $url = $url.'/example.v1.HelloService/Hello';
        } else {
            $url = $url.'/'.$this->prefix.'/example.v1.HelloService/Hello';
        }
    ...
    protected function newRequest(array $ctx, string $url, string $reqBody, string $contentType): RequestInterface
    {
        ...
        return $req
            ...
            ->withHeader('TwirPHP-Version', '0.13.0')
        ;
    }
