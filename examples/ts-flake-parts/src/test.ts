...
test("Enum values and type safety", () => {
  ...
  assert(
    ...
    "Role.MODERATOR should have correct name",
  );
});
...
test("Message serialization and deserialization", () => {
  ...
  assert(
    deserializedUser.id === originalUser.id,
    "Deserialized ID should match",
  );
  assert(
    deserializedUser.email === originalUser.email,
    "Deserialized email should match",
  );
  assert(
    deserializedUser.name === originalUser.name,
    "Deserialized name should match",
  );
  assert(
    deserializedUser.role === originalUser.role,
    "Deserialized role should match",
  );
  assert(
    deserializedUser.isActive === originalUser.isActive,
    "Deserialized active status should match",
  );
  ...
  assert(
    fromJsonUser.email === originalUser.email,
    "From JSON email should match",
  );
});
...
test("Message cloning and equality", () => {
  ...
  assert(
    UserSchema.equals(clonedUser, originalUser),
    "Cloned user should equal original",
  );
  ...
  assert(
    !UserSchema.equals(clonedUser, originalUser),
    "Modified clone should not equal original",
  );
  assert(
    originalUser.name === "Clone Test",
...
test("Service request messages", () => {
  ...
  assert(
    ...
    "CreateUserRequest should have user",
  );
  assert(
    createRequest.user?.id === user.id,
    "CreateUserRequest user ID should match",
  );
  ...
  assert(
    getRequest.id === user.id,
    "GetUserRequest ID should be set correctly",
  );
  ...
  assert(
    ...
    "ListUsersRequest pageSize should be set correctly",
  );
  assert(
    listRequest.pageToken === "",
    "ListUsersRequest pageToken should be set correctly",
  );
  assert(
    listRequest.filter === "role:USER",
    "ListUsersRequest filter should be set correctly",
  );
});
...
test("Optional fields and default values", () => {
  ...
  assert(
    minimalUser.email === "minimal@example.com",
    "Minimal user email should be set",
  );
  assert(
    minimalUser.name === "Minimal User",
    "Minimal user name should be set",
  );
  ...
  assert(
    minimalUser.createdAt === undefined,
    "Default createdAt should be undefined",
  );
  assert(
    minimalUser.updatedAt === undefined,
    "Default updatedAt should be undefined",
  );
});
