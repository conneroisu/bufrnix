...
export type Http = Message<"google.api.Http"> & {
   ...
   * left encoded.
   *
   * The default behavior is to not decode RFC 6901 reserved characters in multi
   * segment matches.
   *
 ...
 * which don't use the URL at all for data exchange, for example:
 *
 *     service Messaging {
 *       rpc GetMessage(GetMessageRequest) returns (Message) {
 *         option (google.api.http) = {
 ...
 *     }
 *
 * This enables the following HTTP JSON to RPC mapping:
 *
 * HTTP | gRPC
 * -----|-----
 * `POST /v1/messages:search { "query": "foo", "page_size": 10, "page_token":
 * "bar" }` | `GetMessage(query: "foo" page_size: 10 page_token: "bar")`
 *
 ...
 * as is. Such variables show up in the
 * [Discovery
 * Document](https://developers.google.com/discovery/v1/reference/apis) as
 * `{+var}`.
 *
 ...
 * have a proto that is reused in multiple APIs/versions or when you don't
 * have control over the proto definition.
 *
 * Example:
 *
 *     http:
 ...
 * proto to JSON conversion must follow the [proto3 JSON
 * specification](https://developers.google.com/protocol-buffers/docs/proto3#json).
 *
 * While the single segment variable follows the semantics of
 * [RFC 6570](https://tools.ietf.org/html/rfc6570) Section 3.2.2 Simple String
 * Expansion, the multi segment variable **does not** follow RFC 6570. The
 * reason is that the multi segment variable is used to capture all path
 * segments of a URL, so that it can be used within the matching algorithm.
 *
...
export type HttpRule = Message<"google.api.HttpRule"> & {
   ...
   * the nesting is only one level deep).
   *
